...
public static class DatabaseManager
{
    ...
    // The primary 
    public static object ReturnQueriedData(DataQueryType type, string abilityName, string queryAttr, string queryCateg)
    {
        return GetXMLData(type, abilityName, queryAttr, queryCateg);
    }

    // Gets the queried data from the XML file.
    static object GetXMLData(DataQueryType type, string abilityName, string queryAttribute, string queryCategory)
    {
        ...
        if (returnedFile != string.Empty)
        {
            ...
            while (reader.Read())
            {
                ...
                if(reader.NodeType == XmlNodeType.Element && reader.LocalName != "AbilityList")
                {
                    while(reader.GetAttribute("Name") != abilityName && reader.ReadToNextSibling("Ability"))
                    {
                        continue;
                    }
                
                    // If it's anything else but the name, you must also have the category filled out.
                    if(queryCategory != string.Empty && queryAttribute != "Name")
                    {
